...
class DUN_RECON(nn.Module):
    def __init__(self, phase=2):
        super(DUN_RECON, self).__init__()
        
        self.phase = phase

        self.ZRD = nn.ModuleList([MST_ZRD() for i in range(self.phase)])

        self.PMM = nn.ModuleList([MST_PMM(in_channels=28, out_channels=28, n_feat=32, stage=1) for i in range(self.phase)])
        
        self.fuse = nn.ModuleList([nn.Conv2d(32 + 28, 28, kernel_size=1, stride=1, padding=0) for i in range(self.phase)])
        
        self.fution = nn.ModuleList([nn.Conv2d(28 + 28, 28, kernel_size=1, stride=1, padding=0) for i in range(self.phase)])
        

    def forward(self, y1, Phi_1, y2, Phi_2):
        rt = None
        xt = None
        de_list = None
        ...
        for i in range(self.phase):
            if i == 0:
                rt_1 = self.ZRD[i](y1, Phi_1, x_pre=None)
                rt_2 = self.ZRD[i](y2, Phi_2, x_pre=None)
                rt = self.fution[i](torch.cat([rt_1, rt_2], dim=1))
                ht, xt, de_list = self.PMM[i](rt, de_list=None)
            else:
                rt_1 = self.ZRD[i](y1, Phi_1, x_pre=xt)
                rt_2 = self.ZRD[i](y2, Phi_2, x_pre=xt)
                rt = self.fution[i](torch.cat([rt_1, rt_2], dim=1))
                rt_fuse = self.fuse[i](torch.cat([rt, ht], dim=1))
                ht, xt, de_list = self.PMM[i](rt_fuse, de_list)   
...
class MaskGenerator(nn.Module):
    def __init__(self, HBW):
        ...
        self.block = nn.Sequential( nn.Conv2d(HBW, HBW, kernel_size=3, stride=1, padding=1),
                                    nn.ReLU(),
...
class PCA_CASSI(nn.Module):
    def __init__(self, stage=2):
        super(PCA_CASSI, self).__init__()
        
        self.stage = stage

        self.mask1 = Parameter(torch.ones(256, 256), requires_grad=True)
        torch.nn.init.normal_(self.mask1, mean=0.5, std=0.1) 
                
        self.static = Parameter(torch.ones(256, 256), requires_grad=True)
        torch.nn.init.normal_(self.static, mean=0.5, std=0.1)

        self.rou = Parameter(torch.zeros(1), requires_grad=True)

        self.R = DUN_RECON(phase=2)
        
        self.M = MaskGenerator(28)
        

    def forward(self, x):
        Phi_1 = torch.sigmoid(self.mask1)
        Phi_1 = Phi_1.contiguous().view(1,1,256,256).repeat(1,28,1,1)
        
        y1 = A(x, Phi_1)
        bs, _, _, _ = y1.shape
        
        
        Phi_2 = self.M(y2x(y1)) * self.rou + self.static.contiguous().view(1,256,256).repeat(bs, 1, 1)
        
        Phi_2 = Phi_2.unsqueeze(1).repeat(1,28,1,1)
                       
        Phi_2 = torch.sigmoid(Phi_2)

        y2 = A(x, Phi_2)

        x2 = self.R(y1, Phi_1, y2, Phi_2)
        
        return x2, Phi_1, Phi_2

if __name__ == '__main__':
    model = MST_2MCA_S2().cuda()
    input = torch.randn(1, 28, 256, 256)
    input = input.cuda()
    out = model(input)
