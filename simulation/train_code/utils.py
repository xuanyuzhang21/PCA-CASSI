...
def generate_dmasks(mask_path, batch_size):
    mask1 = sio.loadmat(mask_path + '/mask.mat')
    mask1 = mask1['mask']
    mask2 = 1 - mask1
    mask3d_1 = np.tile(mask1[:, :, np.newaxis], (1, 1, 28))
    mask3d_1 = np.transpose(mask3d_1, [2, 0, 1])
    mask3d_1 = torch.from_numpy(mask3d_1)
    [nC, H, W] = mask3d_1.shape
    mask3d_batch_1 = mask3d_1.expand([batch_size, nC, H, W]).cuda().float()
    mask3d_2 = np.tile(mask2[:, :, np.newaxis], (1, 1, 28))
    mask3d_2 = np.transpose(mask3d_2, [2, 0, 1])
    mask3d_2 = torch.from_numpy(mask3d_2)
    [nC, H, W] = mask3d_2.shape
    mask3d_batch_2 = mask3d_2.expand([batch_size, nC, H, W]).cuda().float()
    return mask3d_batch_1, mask3d_batch_2

def generate_smasks(mask_path, batch_size):
    mask = sio.loadmat(mask_path + '/mask1.mat')
    mask = mask['mask1']
...
def shuffle_crop_NeRF(train_data, batch_size, crop_size=256):
        index = np.random.choice(range(len(train_data)), batch_size)
        processed_data = np.zeros((batch_size, crop_size, crop_size, 28), dtype=np.float32)
        for i in range(batch_size):
            h, w, _ = train_data[index[i]].shape
            ...
            processed_data[i, :, :, :] = train_data[index[i]][x_index:x_index + crop_size, y_index:y_index + crop_size, :]
        gt_batch = torch.from_numpy(np.transpose(processed_data, (0, 3, 1, 2)))
        gt_batch_small = gt_batch[:,:,::2,::2]
        return gt_batch, gt_batch_small
...
def gen_meas_torch_noise(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    nC = data_batch.shape[1]
    temp = shift(mask3d_batch * data_batch, 2)
    meas = torch.sum(temp, 1)
    if Y2H:
        ...
        meas = np.float32(meas) / np.float32(bit)
        meas = torch.Tensor(meas)

    return meas

def gen_meas_torch_NeRF(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    nC = data_batch.shape[1]
    print(nC)
    print(mask3d_batch.shape, data_batch.shape)
    temp = shift(mask3d_batch * data_batch, 2)
    meas = torch.sum(temp, 1)
    if Y2H:
        meas = meas / nC * 2
...
def init_mask(mask_path, mask_type, batch_size):
    ...
    elif mask_type == 'Mask_small':
        mask3d_batch = mask3d_batch[:, :, ::2, ::2]
...
def init_dmask(mask_path, mask_type, batch_size):
    mask3d_batch1, mask3d_batch2 = generate_dmasks(mask_path, batch_size)
    if mask_type == 'Phi':
        shift_mask3d_batch1 = shift(mask3d_batch1)
        input_mask1 = shift_mask3d_batch1
        shift_mask3d_batch2 = shift(mask3d_batch2)
        input_mask2 = shift_mask3d_batch2
    elif mask_type == None:
        input_mask1 = None
        input_mask2 = None
    else:
        input_mask1 = mask3d_batch1
        input_mask2 = mask3d_batch2
    return mask3d_batch1, mask3d_batch2, input_mask1, input_mask2
...
def init_meas_NeRF(gt, mask, input_setting):
    if input_setting == 'H':
        input_meas = gen_meas_torch_NeRF(gt, mask, Y2H=True, mul_mask=False)
    elif input_setting == 'HM':
        input_meas = gen_meas_torch_NeRF(gt, mask, Y2H=True, mul_mask=True)
    elif input_setting == 'Y':
        input_meas = gen_meas_torch_NeRF(gt, mask, Y2H=False, mul_mask=True)
    return input_meas
